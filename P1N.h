...
public:
  ...
  // return a matrix [a, b; c, d] with det=1 and (c:d) equal to the i'th symbol
  ...
  mat22 lift_to_Gamma_0(long i, const Qideal& M, const Quad& u = Quad::zero, const Quad& v = Quad::zero);

  // each M in GL2 permutes the (c:d) symbols by right multiplcation:
  long apply(const mat22& M, long i);

  // test functions
  void check(int verbose=0);       // checks indexing
  void check_lifts(int verbose=0); // checks lifts to SL2
protected:
  long nrm, phi, psi;
  Qideal N;  // the level
