from sage.all import infinity, copy, NFCusp, Matrix
try:
    assert k
except NameError:
    from Q43 import k, Ok, w, zero, one, emb, rootd, alphas, M_alphas, all_alphas, n_alphas

def to_k(c):
    return k(c.numerator()/c.denominator()) if c.denominator() else oo

def round(z): # z in k
    x, y = ez = emb(z)
    b = (2*y/rootd).round()
    a = ((2*x).round() - b)//2
    return a+b*w

def ispos(a):
    return a[1]>0 or (a[1]==0 and a[0]>0)

def makepos(a):
    return a if ispos(a) else -a

def posgen(I):
    a = I.number_field()(I.gens_reduced()[0])
    return makepos(a)

def posdet(M):
    return makepos(M.det())

def kgcd(a,b):
    return posgen(k.ideal(a,b))

def frac(x):
    """
    Return coprime a,b such that x=a/b.
    """
    if x == infinity:
        return [1,0]
    a = x.numerator()
    b = x.denominator()
    c = kgcd(a,b)
    return [k(a/c),k(b/c)]

def cusp(x):
    """
    Return x as a reduced cusp [a:b], or [1:0] if x is oo.
    """
    return NFCusp(k,infinity) if x==infinity else NFCusp(k,frac(k(x)))

def polygon(xlist):
    """
    Convert a list of k union {oo} to a list of cusps.
    """
    return [cusp(x) for x in xlist]

def edge(x,y):
    """
    Convert a pair of k union {oo} elements to a list of 2 cusps.
    """
    return polygon([x,y])

def is_integral(M):
    """
    Return True iff all entries of M are integral.
    """
    return all(c in Ok for c in M.list())

J = Matrix(2,2,[-1,0,0,1])

def edge_to_mat(e, sign=1):
    """
    Return M such that M(0)=e[0], M(oo)=e[1].  If sign, ensure det(M)
    is "positive".
    """
    M = Matrix(2,2,[e[1].numerator(),e[0].numerator(),e[1].denominator(),e[0].denominator()])
    if sign and M.det()[0]<0:
        M *= J
    return M

def edet(e, pos=False):
    d = e[0].numerator()*e[1].denominator()-e[1].numerator()*e[0].denominator()
    if pos:
        d = makepos(d)
    return d

def apply(U, P):
    try:
        return P.apply(U.list())
    except (AttributeError, TypeError):
        return [c.apply(U.list()) for c in P]

def edge_equiv(e1,e2, sign=1): # e2=U*e1? det(U)=1 only if sign
    """
    Return (True, U) if there exists U in GL(2,O) (or only in SL(2,O)
    if sign) such that U maps e1 to e2, otherwise (False, None).

    """
    d1 = edet(e1)
    d2 = edet(e2)
    if makepos(d1) != makepos(d2):
        return False, None

    M1inv = edge_to_mat(e1).inverse()
    M2 = edge_to_mat(e2)
    if d1 == -d2:
        M2 = M2*J
    U = M2*M1inv
    if is_integral(U):
        return (True, U)
    if not sign:
        U = M2*J*M1inv
        if is_integral(U):
            return (True, U)
    return (False, None)

def std_edge(e, sign=1):
    """
    Return (U,b) if e is the transform under U of b={alpha,oo} for
    one of the alphas in the standard list, else return (False, None).
    If sign, only return (U,b) with det(U)=1.
    """
    d = edet(e, True)
    for alpha in all_alphas:
        b = [alpha,cusp(infinity)]
        if edet(b, True) == d:
            flag, U = edge_equiv(b,e, sign)
            if flag:
                return U,b
    return False, None

def std_poly(P, sign=1):
    """
    Return (U,P') such that U(P')=P, with the first edge of P' a basic
    edge.  If sign, only allow det(U)=1.  If the first edge of P is
    not the transform of a basic edge, return False.
    """
    U, alpha = std_edge(P[:2])
    if not U:
        return False, None
    return U, apply(U.inverse(),P)

def poly_edges(P):
    """
    Return the list of edges of P, including the wrap-around edge from last vertex to first.
    """
    npts = len(P)
    ee = [[P[i],P[i+1]] for i in range(npts-1)] + [[P[npts-1],P[0]]]
    return ee

def cycle_poly(P, n=1):
    """Return a new polygon with the vertices of P cycled round n steps
    (first moved to the end).  Negative n works fine!

    """
    return P[n:] + P[:n]

def reverse_poly(P):
    """
    Return a new polygon with the vertices of P in reverse order.
    """
    return P[::-1]

def poly_equiv(P1,P2, cycle=True, reverse=True, sign=1):
    """
    Test whether the polygons P1, P2 are GL(2,O)-equivalent (of
    SL(2,O)-equivalent if sign).  If cycle is True (default), allow P2
    to be cycled, i.e. test if P1 is equivalent to a cycled version of
    P2.  If reverse is True (default), allow P2 to be reversed.

    So for triangles, with the defaults, up to 6 exact transforms are
    tested.

    """
    npts = len(P1)
    if len(P2) != npts:
        return (False, None)

    if reverse:
        flag, U = poly_equiv(P1, P2, cycle, False, sign)
        if flag:
            return flag, U
        flag, U = poly_equiv(P1, reverse_poly(P2), cycle, False, sign)
        return flag, U

    P3 = copy(P2) # copy for cycling and reversing without changing P2
                  # itself
    e1 = P1[:2] # first edge of P1
    for i in range(npts):
        e2 = P3[:2] # first edge of P3
        flag, U = edge_equiv(e1,e2, sign)
        if flag:
            if apply(U,P1)==P3:
                return (True, U)
        # now possibly cycle P3
        if i+1<npts and cycle:
            print("not yet, cycling")
            P3 = cycle_poly(P3)
        else:
            return (False, None)
    return (False, None)

def is_cyclic(T):
    return poly_equiv(T, cycle_poly(T), False)

def add_alpha(a,b,c,d):
    global M_alphas, all_alphas, n_alphas
    assert a*d-b*c==1
    M_alphas.append(Matrix(2,2,[a,b,c,d]))
    all_alphas.append(cusp(-d/c))
    n_alphas += 1

def add_two_alphas(s, r): # r^2=-1 (mod s)
    t = -(r*r+1)/s
    add_alpha( r, t, s, -r) # alpha =  r/s
    add_alpha(-r, t, s,  r) # alpha = -r/s

def add_four_alphas(s, r1, r2): # r1*r2=-1 (mod s)
    t = -(r1*r2+1)/s
    add_alpha( r2, t, s, -r1) # alpha =  r1/s
    add_alpha(-r2, t, s,  r1) # alpha = -r1/s
    add_alpha( r1, t, s, -r2) # alpha =  r2/s
    add_alpha(-r1, t, s,  r2) # alpha = -r2/s

tri0 = [cusp(0),cusp(infinity),cusp(1)]

def make_triangles():
    Tlist = []
    for i in range(n_alphas):
        Ma = M_alphas[i]
        for j in range(n_alphas):
            beta = all_alphas[j]
            gamma = apply(Ma,beta)
            if gamma == NFCusp(k,oo):
                pass
            else:
                try:
                    kkk=next(kk for kk in range(n_alphas) if to_k(gamma)-to_k(all_alphas[kk]) in Ok)
                    print("({},{},{}) : <{},{},{}>".format(i,j,kkk,to_k(all_alphas[i]), oo, to_k(all_alphas[j])))
                    Tlist.append((i,j,kkk))
                except StopIteration:
                    pass
    return Tlist, [[all_alphas[i],NFCusp(k,oo),all_alphas[j]] for i,j,_ in Tlist]

def all_perms(T):
    ans = [cycle_poly(T,n) for n in range(len(T))]
    ans = ans + [reverse_poly(t) for t in ans]
    ans = ans + [apply(J,t) for t in ans]
    return [std_poly(t)[1] for t in ans]

def check_triangle_in_list(T, triangles):
    return any(t in triangles for t in all_perms(T))

def reduce_triangles(Tlist, triangles):
    Tlist0 = []
    triangles0 = []
    for trip, tri in zip(Tlist, triangles):
        if not check_triangle_in_list(tri, triangles0):
            Tlist0.append(trip)
            triangles0.append(tri)
    return Tlist0, triangles0
