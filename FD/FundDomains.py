...
def fundamental_domain(k):
    ...
    Each element of V is a triple (x,y,t2) of floats where x+y*w
    approximates an element of K and t2 is a float approximation to
    the square of the t-coordinate (so x, y, t2 are rational).
...
def plot_Bianchi_diagram(k, S):
    ...
    for s in BS:
        ...
        L.append(implicit_plot3d(eq, (Y, -Yrange, Yrange ),  (X, -0.5, 0.5), \
...
def elements_of_norm(k,i):
    """
    Returns a list of elements of k of positive norm i, for k=Q(sqrt(-d)) imaginary
    quadratic field with d>3. The list contains both 'x' and '-x'.
    """
    return sum([[l,-l] for l in k.elements_of_norm(i)], [])
...
def list_of_bounded_elems(k, minB, maxB):
    ...
    """
    return sum([elements_of_norm(k,i) for i in range(minB + 1,maxB + 1)], [])

def in_circle(k, P, s, strict=False):
    """

    P = alpha or (P0,P1): a point in k, either an actual element of k,
    or a pair of rational coordinates w.r.t. basis [1,sqrt(-d)] of k.

    s = [alpha,ir2]: a circle (alpha in k, ir2 the inverse square radius)

    Output: True iff P is inside s, (strictly inside if strict is
    True, else inside or on s).

    """
    rad2 = 1/s[1] #this rad2 is the square of the radius, and is rational
    try:
        dist = (k(P)-s[0]).norm()
    except TypeError:
        D = k.absolute_generator().norm()
        dist = ((s[0][0]-P[0])**2 + (s[0][1]*sqrt(D) - P[1])**2)
    return dist < rad2 if strict else dist <= rad2

def find_circles(k, P, Slist, strict=False):
    """
    P: a point in k
    Slist: a list of circles

    Output: True iff P is inside (or strictly inside) all s in Slist
    """
    return [s for s in Slist if in_circle(k, P, s, strict)]

def hem_covered_by_hem(k, s1, s2):
    """
    s1, s2: hemispheres (alpha, ir2)

    Return True iff s1 in covered by s2
    """
    # first check if the centre of s1 is contained in s2
    # (in fact in the projection of 'l' on the plane).
    if not in_circle(k, s1[0], s2, strict=True):
        return False
    rad1 = 1/RR(sqrt(s1[1]))
    rad2 = 1/RR(sqrt(s2[1]))
    return (s1[0] - s2[0]).norm() <= (rad1 - rad2)**2

def hem_covered(k, s, Slist):
    """
    s = (alpha, ir2): hemisphere

    Slist: list of hemispheres

    Return True iff s in covered any s' in Slist
    """
    return any(hem_covered_by_hem(k, s, s2) for s2 in Slist)
...
def initial_list(k):
    ...
    Input: an imaginary quadratic field.

    Output: a list of "hemispheres" S_alpha: each hemisphere is
    encoded as pair (alpha,r2) where alpha is in k and r2 is the norm
    of the denominator ideal of alpha, so 1/r2 is the square of the
    radius of S_alpha.
...
def singular_points(k):
    ...
    Algorithm: Swan [Theorem 7]

    The returned values are tuples of rationals (a,b) representing
    a+b*sqrt{-d} (with -d squarefree), whatever the basis for k is.

    """
    L = []
    d = -k.discriminant().squarefree_part()
    smax = (RR(4*d)/3).sqrt().ceil()
    step = ZZ(2) if d.mod(4) == 3 else ZZ(1)
    for s in xsrange(2*step, smax, step):
        #print("s={}".format(s))
        for r in xsrange(-s//2 + 1, s//2 + 1):
            #print(" - r={}".format(r))
            r2d = r*r+d
            if s*s <= r2d and (step*s).divides(r2d):
                s0 = s/step
                #print(" - - s0={}".format(s0))
                L1 = [(p*r/s, p/s) for p in xsrange(s0) if p.gcd(s0)==1]
                #print("*** {}".format(L1))
                L += L1
    return L

# Fundamental rectangle: {x+y*i: -1/2<x<=1/2, -sqrt|D|/4<y<=sqrt|D|/4}
# F_k is the half of this with y>=0

def reduce_ab_mod_ok(k, ab):
    """
    ab=(a,b) coords of z=a+b*sqrt{-d} in k, returns integer translate
    (a0,b0) lying in fundamental rectangle.
    """
    a,b = ab
    half = Integer(1)/Integer(2)
    if k.discriminant()%4==0:
        # Here w = sqrt(-d) so a+b*sqrt(-d) = a+b*w
        # and we want a and b to be in (-1/2,1/2]
        b -= (b-half).ceil()
        a -= (a-half).ceil()
        assert -half < a <= half
        assert -half < b <= half
    else:
        # Here w = (1+sqrt(-d))/2 so a+b*sqrt(-d) = (a-b)+2*b*w
        # and we want a-b and 2b to be in (-1/2,1/2]
        c = (2*b-half).ceil()/2
        e = (a-b-half).ceil()
        b -= c
        a -= (c+e)
        assert -half < a-b <= half
        assert -half < 2*b <= half
    return (a,b)
...
def singular_points_in_F(k):
    ...
    fundamental region.
    """
    return [ab for ab in [reduce_ab_mod_ok(k,ab) for ab in singular_points(k)] if ab[1]>=0]

def singular_points_in_F_old(k):
    """
    Returns a list with the singular points which are on the floor of the 
    fundamental region.
    """
    ...
    for P in L: #we test if P or a translated P is inside Fk
        sign = 1
        t_rang = srange(int(P[0] - 1/2), int(P[0] + 1))
        if P[0]<0:
            sign = -1
            t_rang = srange(int(sign*(P[0])), int(sign*(P[0] - 1/2) + 1))
        for k in srange(int(-d*(h - P[1])), int(d*P[1] + 1)):
            kd = k//d
            if 0< (P[1] - kd) <= h:
                for t in t_rang:
                    if kd <= (P[0] - sign*t) <= (1/2 + kd):
                        LinF.append((P[0] - sign*t - kd, P[1] - kd))
    return LinF
...
def list_rectangles(k, sk, S):
    ...
    """
    h = 1/4 if k.discriminant().mod(4)==1 else 1/2
    return subdivide_rect(k, (0, 0), 1/2, h, sk, S)
...
def circles_intersection_check(k, c1, c2):
    ...
    return RR(c_distance) < RR(sqrt(rad1)) + RR(sqrt(rad2))
...
def clean_list(k, S):
    ...
    Input: a list S, of principal hemispheres whose translates coever
    the floor (apart from singular points).

    Output: (V, S') where S' is a sublist of S which also covers, and
    V is a list of interior points where at least 3 hemispheres
    intersect.
