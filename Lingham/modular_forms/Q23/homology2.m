...
function DimH(level_g,flag)  // when flag eq 0 outputs dimension of the 
                             // homology space. If flag eq 1 then all
                             // intermediate data is also outputed
   
   KI_g:=quo<OK|level_g>;
   PKI_g:=ProjectiveSpace(KI_g,1);
   MKI_g:=RMatrixSpace(KI_g,2,2);

   assert level_g ne 1*OK;  // check the level isn't trivial  
 
   CDlist:=List([s[1]^s[2]:s in Factorization(level_g)],level_g,KI_g,PKI_g);

   cdn:=#CDlist;
   
   crep:=[ToMatrix(mark,level_g,KI_g,PKI_g):mark in CDlist];
