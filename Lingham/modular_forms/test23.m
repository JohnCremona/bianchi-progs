...
time S2:=Check(S1);
assert #S2 eq 15;
assert S2 eq { <0, 1>, <w - 2, w + 1>, <1, -1>, <w + 1, 3>, <2*w - 1, 4>, <3, -w + 2>, <1, 1>, <-1, 1>,
    <2*w - 3, 4>, <2*w + 1, 4>, <w + 2, -w + 3>, <-3, w + 1>, <w - 3, w + 2>, <w + 1, -w + 2>, <w - 2, 3>};


S2a:=[Cusps![x[1],x[2]]:x in S2]; // as a list of cusps
S2b:=[x[1]/x[2]:x in S2a]; // as a list of elements of K

time S3,I1,SP1:=Inter(S2);
assert S2 eq S3;
assert #I1 eq 22;
assert SP1 eq { w/2, (w-1)/2};
...
assert #VO eq 3;
RP:=RepPoly(VO,S2);                     // Representative polyhedra
